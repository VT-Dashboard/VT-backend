...
BEGIN
  ...
  IF NOT EXISTS (SELECT * FROM sys.foreign_keys WHERE parent_object_id = OBJECT_ID('dbo.products') AND name = 'FK_products_brand_id')
    ALTER TABLE dbo.products ADD CONSTRAINT FK_products_brand_id FOREIGN KEY (brand_id) REFERENCES dbo.brands(id);
END
